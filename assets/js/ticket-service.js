(function () {
  function ensureOk(res, action) {
    if (!res || !res.ok) throw new Error(`${action} failed: ${res ? res.status : "no_response"}`);
    return res;
  }
  ...
  async function loadTickets(searchParams) {
    ...
    const res = await authedFetch(`/api/tickets?${query}`, { cache: 'no-store' });
  ...
  async function createTicket(payload) {
    ...
    ensureOk(res, 'create ticket');
    return await res.json().catch(() => ({}));
  }

  async function updateTicket(id, payload) {
    const res = await authedFetch(`/api/tickets/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload || {})
    });
