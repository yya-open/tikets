...
export function normalizeImportRecord(record, forcedDeleted = null) {
  ...
  return {
    ...
    status: String(obj.status ?? obj.ticketStatus ?? "待处理").trim() || "待处理",
    priority: String(obj.priority ?? "普通").trim() || "普通",
    assignee: String(obj.assignee ?? obj.owner ?? "").trim(),
    due_date: String(obj.due_date ?? obj.dueDate ?? "").trim(),
    closed_at: String(obj.closed_at ?? obj.closedAt ?? "").trim(),
    updated_at,
    updated_at_ts,
    is_deleted,
    deleted_at,
